...
DWORD WINAPI ThreadRead( LPVOID lParam)
{
	...
	READ_ACCESS_PARAMS rParams = { MEMORY_BANK_RESERVED,0,8,0,0 };
	{
		...
		Print2Console(pMyTagData->antennaID,pMyTagData->CRC,pMyTagData->PC, pMyTagData->XPC,L" Thread Read on MEMORY_BANK_RESERVED" ,pMyTagData->pTagID,pMyTagData->tagIDLength );
	}
	{
		rParams.memoryBank = MEMORY_BANK_TID;
		api3Data.rfidStatus = RFID_Read( api3Data.hReader,pMyTagData->pTagID,pMyTagData->tagIDLength,&rParams,NULL,&api3Data.pInfo,pMyTagData,0);
		Print2Console( pMyTagData->antennaID,pMyTagData->CRC,pMyTagData->PC, pMyTagData->XPC,L" Thread Read on MEMORY_BANK_TID" ,pMyTagData->pTagID,pMyTagData->tagIDLength );
	}
	{
		rParams.memoryBank = MEMORY_BANK_USER;
		api3Data.rfidStatus = RFID_Read( api3Data.hReader,pMyTagData->pTagID,pMyTagData->tagIDLength,&rParams,NULL,&api3Data.pInfo,pMyTagData,0);
		Print2Console( pMyTagData->antennaID,pMyTagData->CRC,pMyTagData->PC, pMyTagData->XPC,L" Thread Read on MEMORY_BANK_USER",pMyTagData->pTagID,pMyTagData->tagIDLength );
	}
...
DWORD WINAPI ThreadUnLock( LPVOID lParam )
{
	...
	READ_ACCESS_PARAMS rParams = { MEMORY_BANK_RESERVED,4,4,0,0};
...
void TestThreadSafeSingleReader( void )
{
	...
	{		
		HANDLE h1,h2,h3;
		h3 = CreateThread(NULL,0,ThreadUnLock,(LPVOID)pgTagData,0,&dwThreadID);
		h1 = CreateThread(NULL,0,ThreadRead,(LPVOID)pgTagData,0,&dwThreadID);
		h2 = CreateThread(NULL,0,ThreadWrite,(LPVOID)pgTagData,0,&dwThreadID);
		pgTagData = &(TagData[Cnt++]);
		pgTagData= RFID_AllocateTag( api3Data.hReader);
	}
	...
	{
		pgTagData = &(TagData[Cnt]);
		if( pgTagData != NULL ) RFID_DeallocateTag( api3Data.hReader, pgTagData );
		Cnt++;
